...
{
	function get_all(){
		$this->db->from('shippings sh');
		$this->db->join('sales sa','sh.sale_id=sa.sale_id');
		$this->db->order_by("ship_id");
		
		return $this->db->get();
	}

	function get_item_info_by_sale_id($sale_id){
		$this->db->select('i.*');
		$this->db->from('sales_items si');
		$this->db->join('items i', 'si.item_id=i.item_id');
		$this->db->where('si.sale_id', $sale_id);
		$this->db->order_by('si.item_id');
	...
	function get_search_invoice_number($invoice){
		$this->db->from('sales');
		$this->db->like('invoice_number', $invoice);

		return $this->db->get();
	}

	public function save(&$shipping_data, $ship_id=false)
	{
		if (!$ship_id or !$this->exists($ship_id))
		{
			if($this->db->insert('shippings',$shipping_data))
			{
				$shipping_data['ship_id']=$this->db->insert_id();
				return true;
			}
			return false;
		}
		...
		return $this->db->update('shippings', $shipping_data);
	}
 ...
 //                                              'location_id'=>$item['item_location']), $item['item_id'], $item['item_location']);
	...
	function delete_list($shipping_ids)
	{
		$this->db->where_in('ship_id',$shipping_ids);
