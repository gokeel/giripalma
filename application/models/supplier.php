...
{	
	...
	*/
	function delete_list($supplier_ids)
	{
 	...
 	/*
	Get earch suggestions to find suppliers
	*/
	function get_search_suggestions($search,$limit=25)
	{
		...
		CONCAT(`first_name`,' ',`last_name`) LIKE '%".$this->db->escape_like_str($search)."%') and deleted=0 and (company_name LIKE '%".$this->db->escape_like_str($search)."%' OR agency_name LIKE '%".$this->db->escape_like_str($search)."%' OR account_number LIKE '%".$this->db->escape_like_str($search)."%')");
		$this->db->order_by("company_name", "asc");
